...
<div class='allorders mt-5 pt-4'>
    <div class="container-fluid">
        ...
        <div class="allorders__body">
            ...
            <div class="row gy-2">
                <div class="col-sm-12" v-for='(companion, index) in orders' :key="index">
                    <div class="card">
                        <div class="row g-0">
                            ...
                            <div class="col-9">
                                <div class="card-body">
                                    <h5 class="card-title mb-2">{{ companion.from + ' - ' + companion.to }}</h5>
                                    <p class="card-text mb-0"><small class='text-muted'>Время: <span class='font-weight-bold'>{{ companion.date | moment(companion.date)}} </span></small></p>
                                    <p class="card-text mb-0"><small class='text-muted'>Вид посылки: <span class='font-weight-bold'>{{ companion.typePackage }}</span></small></p>
                                    <p class="card-text"><small class="text-muted">Цена: <span class='font-weight-bold'>{{ companion.price }} тг. </span></small></p>
                                </div>
                            </div>
                        </div>
                        <!--
...
export default {
    ...
    methods: {
        searchChoose() {
            axios.get(`api/client-orders/${this.from}/${this.to}`)
                .then(res => {
                    if (res.status === 200) {
                        console.log(res)
